...
class CLI:
    ...
    async def query(
        self, question: str, provider: str | None = None, document: str | None = None
    ):
        """Execute a query."""
        ...
        if result.get("sources"):
            ...
            for i, source in enumerate(result["sources"], 1):
                console.print(
                    f"  {i}. {source['source']} " f"(Page {source.get('page', 'N/A')})"
                )
    ...
    def show_config(self):
        ...
        for name, cfg in llm_loader.providers.items():
            is_default = (
                "✓"
                if llm_loader.default and llm_loader.default.provider == cfg.provider
                else ""
            )
